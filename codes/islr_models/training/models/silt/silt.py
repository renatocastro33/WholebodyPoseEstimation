...
class SILT(nn.Module):
    ...
    def __init__(self, num_classes,hidden_dim=108, num_heads=9, num_layers_1=6, num_layers_2=6, 
        ...
        self.importance_points = nn.Parameter(torch.ones(1,1, hidden_dim)) #before it was called self.pos > careful when trying to load them

        custom_decoder_layer = SPOTERTransformerDecoderLayerBase(self.transformer.d_model, self.transformer.nhead,
                                                             dim_feedforward_decoder, dropout=dropout, activation="relu",batch_first=batch_first)

        self.transformer.decoder.layers = _get_clones(custom_decoder_layer, self.transformer.decoder.num_layers)

    def forward(self, inputs,padding_mask=None):
        batch_size, seq_len, feat_dim = inputs.size()
        class_query = self.class_query.expand(batch_size, -1, -1)# shape: (batch_size, 1, hidden_dim)
        #print("")
        #print("self.importance_points:",self.importance_points)
        explainabInputsEnc = inputs*self.importance_points
        h = self.transformer(
                            src=explainabInputsEnc,
                             tgt=class_query,
